...
class CmTrace(object):
    ...
    def dump(self):
        sep = '|'
        print(f"{'index':>6}{sep}{'PC':>10}{sep}{'opcode':>6}{sep}{'cycles':>6}{sep}{'cycles sum':>10}")
        index = 1
        scy = 0
        for r in self.get_records():
            scy += r['cycles']
            print(f"{index:6}{sep}0x{r['pc']:08x}{sep}{self._image.addresses[r['pc']]['ins']:>6}{sep}{r['cycles']:6}{sep}{scy:10}")
            index += 1
    ...
    def from_file(trace_path):
        with open(trace_path,'rb') as f:
            ...
            if ins_cnt != trace._ins_cnt:
                raise RuntimeError(f'File {trace_path} seems corrupted: inconsistent size vs number of instruction ({trace._ins_cnt} vs {ins_cnt})')
            return trace
    ...
    def capture(self,device,*, out_dir=None):
        ...
        with open(trace_path,'wb') as f:
            ...
            def add_instruction():
                ...
                if is_setup:
                    ins_cnt_setup += 1
                    logging.debug(f'PC={last_pc:#08x}: {last_pc_cycles} {self._image.addresses[last_pc]['ins']} (setup)')
                else:
                    ins_cnt += 1
            ...
            print(f'{self._func_name}: {ins_cnt} instructions, {func_cycles} cycles')
            f_write32(ins_cnt)
